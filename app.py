...
def index():
    ...
    if busqueda_actual:
        condiciones.append("(lower(t.codigo) LIKE ? OR lower(c.nombres) LIKE ? OR lower(c.dni) LIKE ?)")
        termino = f"%{busqueda_actual.lower()}%"
    ...
    with conectar_db() as conn:
        tramites = conn.execute(consulta, parametros).fetchall()
...
def registrar():
    if request.method == "POST":
        ...
        with conectar_db() as conn:
            ...
            cursor = conn.execute(
                ...
                INSERT INTO tramites (
                    ...
                    probabilidad, estado, observaciones, fecha_registro
                )
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
        ...
        flash(f"Trámite registrado. Riesgo: {riesgo}.", "success")
...
@app.route("/tramite/<int:tramite_id>/editar", methods=["GET", "POST"])
def editar_tramite(tramite_id: int):
    with conectar_db() as conn:
        tramite = conn.execute(
            """
            SELECT t.*, c.dni, c.nombres, c.telefono, c.correo, c.direccion
    ...
    if request.method == "POST":
        ...
        if not dni or not nombres:
            flash("Debe ingresar DNI y nombres del ciudadano.", "error")
            return redirect(url_for("editar_tramite", tramite_id=tramite_id))
        ...
        try:
            with conectar_db() as conn:
                conn.execute(
                    """
                    UPDATE ciudadanos
                    SET dni = ?, nombres = ?, telefono = ?, correo = ?, direccion = ?
                    WHERE id = ?
                    """,
                    (dni, nombres, telefono, correo, direccion, tramite["ciudadano_id"]),
                )
                conn.execute(
                    """
                    UPDATE tramites
                    SET tipo_tramite = ?, area = ?, documentos_completos = ?,
                        num_derivaciones = ?, dias_transcurridos = ?, carga_area = ?,
                        riesgo_retraso = ?, probabilidad = ?
                    WHERE id = ?
                    """,
                    (
                        datos_tramite["tipo_tramite"],
            ...
            flash("El DNI ingresado ya está registrado en otro ciudadano.", "error")
            return redirect(url_for("editar_tramite", tramite_id=tramite_id))
...
@app.route("/tramite/<int:tramite_id>/eliminar", methods=["POST"])
def eliminar_tramite(tramite_id: int):
    with conectar_db() as conn:
        tramite = conn.execute("SELECT id FROM tramites WHERE id = ?", (tramite_id,)).fetchone()
        if not tramite:
            flash("Trámite no encontrado.", "error")
            return redirect(url_for("index"))

        conn.execute("DELETE FROM historial WHERE tramite_id = ?", (tramite_id,))
        conn.execute("DELETE FROM tramites WHERE id = ?", (tramite_id,))
        conn.commit()
